...
def center_branch_simple_model(dim_left, dim_right, dim_dense = 2048, activation = "relu", bias = False):
  ...
  left_branch = Dense(dim_dense, input_dim=4096, name='left_branch', activation=activation, bias = bias)(center_left)

  right_branch = Dense(dim_dense, input_dim=768, name='right_branch', activation=activation, bias = bias)(center_right)
